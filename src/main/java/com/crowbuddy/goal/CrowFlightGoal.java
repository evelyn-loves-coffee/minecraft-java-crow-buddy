...
public class CrowFlightGoal extends Goal {

    private static final double TAKEOFF_VELOCITY = 0.45;
    private static final double LIFT_VELOCITY = 0.06;
    private static final double MAX_FLIGHT_HEIGHT = 24.0;
    private static final int LIFT_INTERVAL = 3;
    ...
    public void stop() {
        ...
        this.flightCooldown = reducedTickDelay(40);
    }
    ...
    public void tick() {
        ...
        if (this.crow.isAirborne()) {
            ...
            if (this.chaseTarget != null && this.chaseTarget.isAlive()) {
                this.navigateTowards(this.chaseTarget.getX(), this.chaseTarget.getY() + 2.0, this.chaseTarget.getZ());
            }
            if (this.crow.getY() > MAX_FLIGHT_HEIGHT) {
                Vec3 cur = this.crow.getDeltaMovement();
                this.crow.setDeltaMovement(cur.add(0.0, -0.04, 0.0));
            }
        }
    ...
    private boolean shouldContinueFlight() {
        ...
        if (owner != null && owner.isAlive()) return this.crow.distanceToSqr(owner) > 256.0;
        return false;
    }
    ...
    private boolean ownerMovingAway() {
        ...
        return speed > 0.15 && distSq > 144.0;
    }

    private boolean hasNearbyThreat() {
        double rSq = 16.0;
        for (LivingEntity e : this.crow.level().getEntitiesOfClass(LivingEntity.class, this.crow.getBoundingBox().inflate(rSq, 4.0, rSq))) {
    ...
    private void findChaseTarget() {
        LivingEntity owner = this.crow.getOwner();
        if (owner != null && owner.isAlive() && this.crow.distanceToSqr(owner) > 100.0) {
            this.chaseTarget = owner;
            return;
        }
        for (LivingEntity e : this.crow.level().getEntitiesOfClass(LivingEntity.class, this.crow.getBoundingBox().inflate(16.0, 4.0, 16.0))) {
            if (e == this.crow || e == this.crow.getOwner()) continue;
    ...
    private void takeOff() {
        ...
        this.crow.setDeltaMovement((cur.x / len) * 0.2, TAKEOFF_VELOCITY, (cur.z / len) * 0.2);
    ...
    private void land() {
        ...
        if (!this.crow.level().getBlockState(below).getCollisionShape(this.crow.level(), below).isEmpty()) {
            ...
            if (Math.abs(this.crow.getY() - top) < LAND_THRESHOLD) {
                Vec3 cur = this.crow.getDeltaMovement();
                if (cur.y < 0.0) this.crow.setDeltaMovement(cur.add(0.0, 0.15, 0.0));
    ...
    private void navigateTowards(double x, double y, double z) {
        ...
        if (len < 0.1) return;
        double nx = cur.x + (dx / len) * 0.08;
        double ny = cur.y + (dy / len) * 0.04;
        double nz = cur.z + (dz / len) * 0.08;
        double hSpeed = Math.sqrt(nx * nx + nz * nz);
        if (hSpeed > 0.4) {
            double scale = 0.4 / hSpeed;
            nx *= scale;
            nz *= scale;
        }
