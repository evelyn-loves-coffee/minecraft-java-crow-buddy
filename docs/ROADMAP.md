...
| Phase 3: Networking & Core Mechanics | ✅ Complete | 9/9 |
...
*   ~~Event Callbacks — `ServerLivingEntityEvents.AFTER_DAMAGE` + `AttackEntityCallback.EVENT` registered in `CrowEventHub`. 4 handlers wired to `SwarmManager.INSTANCE` for swarm/retaliation dispatch.~~
...
*   ~~Swarm Intelligence — `SwarmManager` dispatcher + `SwarmDistressGoal` (priority 0 on `goalSelector`). 4 `CrowEventHub` stubs wired to `SwarmManager`. Untamed attacked → immediate swarm (source + 5 nearest, 32-block radius). Tamed attacked → 1st hit: retaliation (~2s), 3 hits in 30s: escalation to full swarm. Owner attacks hostile mob → swarm on that mob. Hostile mobs → indefinite targeting. Players → 4s sliding window. All 6 crows play distress with dual-layer variance every 20 ticks. 15s per-crow cooldown. Sit cancels. `PathNavigation` ground pathfinding.~~
